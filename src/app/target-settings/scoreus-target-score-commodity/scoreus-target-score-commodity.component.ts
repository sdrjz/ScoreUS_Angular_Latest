...
@Component({
  selector: 'app-scoreus-target-score-commodity',
  templateUrl: './scoreus-target-score-commodity.component.html',
  styleUrls: ['./scoreus-target-score-commodity.component.css']
})
export class ScoreusTargetScoreCommodityComponent implements OnInit {
  ...
  getProjectsUrl:string = api.commodityTargetSetting
  poRowcolumns = [
    {
      def: 'plantCode',
      name: 'Plant Code',
  ...
  onDelete(data:any){
    ...
    this._apiService.get(`${api.deleteDataFromTargetAndRaw}/CommodityTarget/${data.id}`)
    .subscribe((res:any)=>{
...
onUpdate(data:any){
  ...
  dialogRef.afterClosed().subscribe((res: any) => {
    ...
    this._apiService.isCompareLoader$.next(true);
    this._apiService.post(api.updateCommodityTarget,res)
