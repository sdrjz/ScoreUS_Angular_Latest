...
@Component({
  selector: 'app-scoreus-target-score-buyer',
  templateUrl: './scoreus-target-score-buyer.component.html',
  styleUrls: ['./scoreus-target-score-buyer.component.css']
})
export class ScoreusTargetScoreBuyerComponent implements OnInit {
  ...
  getProjectsUrl: string = api.buyerTargetSetting
  poRowcolumns = [
    {
      def: 'plantCode',
      name: 'Plant Code',
  ...
  onDelete(data:any){
    ...
    this._apiService.get(`${api.deleteDataFromTargetAndRaw}/BuyerTarget/${data.id}`)
    .subscribe((res:any)=>{
 ...
 onUpdate(data:any){
  ...
  dialogRef.afterClosed().subscribe((res: any) => {
    ...
    this._apiService.isCompareLoader$.next(true);
    this._apiService.post(api.updateBuyerTarget,res)
