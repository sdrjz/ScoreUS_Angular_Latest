...
@Component({
  selector: 'app-scoreus-target-score-materials',
  templateUrl: './scoreus-target-score-materials.component.html',
  styleUrls: ['./scoreus-target-score-materials.component.css']
})
export class ScoreusTargetScoreMaterialsComponent implements OnInit {
  ...
  getProjectsUrl:string = api.materialTargetSetting
  ...
  onDelete(data:any){
    ...
    this._apiService.get(`${api.deleteDataFromTargetAndRaw}/MaterialTarget/${data.id}`)
    .subscribe((res:any)=>{
...
onUpdate(data:any){
  ...
  dialogRef.afterClosed().subscribe((res: any) => {
    ...
    this._apiService.isCompareLoader$.next(true);
    this._apiService.post(api.updateMaterialTarget,res)
