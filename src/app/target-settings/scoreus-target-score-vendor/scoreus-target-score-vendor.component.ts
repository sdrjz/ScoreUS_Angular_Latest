...
@Component({
  selector: 'app-scoreus-target-score-vendor',
  templateUrl: './scoreus-target-score-vendor.component.html',
  styleUrls: ['./scoreus-target-score-vendor.component.css']
})
export class ScoreusTargetScoreVendorComponent implements OnInit {
  ...
  getProjectsUrl:string = api.vendorTargetSetting
  poRowcolumns = [
    {
      def: 'vendorCode',
      name: 'Vendor Code',
  ...
  onDelete(data:any){
    ...
    this._apiService.get(`${api.deleteDataFromTargetAndRaw}/VendorTarget/${data.id}`)
    .subscribe((res:any)=>{
...
onUpdate(data:any){
  ...
  dialogRef.afterClosed().subscribe((res: any) => {
    ...
    this._apiService.isCompareLoader$.next(true);
    this._apiService.post(api.updateVendorTarget,res)
