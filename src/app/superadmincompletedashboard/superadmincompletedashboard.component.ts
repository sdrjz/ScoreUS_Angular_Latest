...
@Component({
  selector: 'app-superadmincompletedashboard',
  templateUrl: './superadmincompletedashboard.component.html',
  styleUrls: ['./superadmincompletedashboard.component.css']
})
export class SuperadmincompletedashboardComponent implements OnInit {
  ...
  getAllDashBoardData(){
    this._apiService.isCompareLoader$.next(true)
    Promise.all([
      this._apiService.get(`${api.superAdminDashboardProduct}?startDate=${this.startDate}&endDate=${this.endDate}&type=3`).toPromise(),
